...
export class CSSModuleLoaderProcess {
  ...
  _fetchDependencies (_newPath, relativeTo) {
    ...
    let url = new URL(System.baseURL, true)
    url.set('pathname', path.join(url.pathname, canonicalParent))
    const parentName = url.href

    return System.normalize(newPath + '!', parentName)
      .then((importPath) => {
